...
preg_match_all('/\(([0-9-]+)\)[\s]+(.+)/', copyPasteSite(), $m);
...
file_put_contents('./CurlybErrorMessages.php', $code);



function formatLine($code, $message)
{
  return sprintf("%d => '%s'", $code, addslashes($message));
}

function copyPasteSite()
{
